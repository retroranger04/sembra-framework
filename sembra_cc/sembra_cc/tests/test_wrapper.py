"""Tests for the charge-controlled wrapper."""
...
from sembra_cc import wrapper as wrapper_module
...
def test_no_equilibrium_returns_p_crit_boundary_equilibrium():
    ...
    from sembra_cc.symbolic import evaluate_gradient
...
def test_no_equilibrium_adaptive_margin_recovers():
    ...
    result = predict(2.0, 0.40, 3.0)  # p >> p_crit; solve at p_crit fails, retreats
    diag = result.diagnostics
    assert diag["branch"] == "no_equilibrium"
...
def test_near_fold_returns_converged_equilibrium():
    ...
    from sembra_cc.envelope import load_envelopes, predict_p_crit
...
def test_stable_branch_end_to_end():
    ...
    import sembra_cc.wrapper as wrap
    from sembra_cc.envelope import load_envelopes, predict_p_crit
    from sembra_cc.network import load_network
