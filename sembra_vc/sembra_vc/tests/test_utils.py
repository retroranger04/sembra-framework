"""Tests for the voltage-controlled utility helpers."""

import numpy as np

from sembra_vc.calibrations import STRESS_DISPLAY_GRID_SIZE
...
def test_compute_stress_pole_value():
    """At r=0 (equibiaxial pole) t1 == t2 == the closed-form VC value.

    lam1 = lam2 = lambda_0 and lam3 = 1/lambda_0^2 there, so (Eq. 17)
...
def test_compute_max_electric_field_vc():
    """VC E_max = lambda_0**2."""
    state = (1.4, 0.05, 0.3, -0.05)
    assert compute_max_electric_field(state) == np.float64(1.4**2)
...
from sembra_vc.utils import (  # noqa: E402
    format_prediction_report,
    plot_design_space,
    plot_stress_field,
)
from sembra_vc.wrapper import PredictionResult  # noqa: E402
...
def test_report_stable_branch_labels_and_residual_framing():
    ...
    assert "SEMBRA — Voltage-Controlled" in report
...
def test_plot_stress_field_returns_figure_and_saves(tmp_path, monkeypatch):
    ...
    import sembra_vc.utils as utils_mod
