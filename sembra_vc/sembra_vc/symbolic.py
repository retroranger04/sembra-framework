"""Symbolic/numerical energy core for the voltage-controlled (VC) scenario.
...
def _build_symbolic_core():
    ...
    # Energy densities (paper Eqs. 2, 4, 5).
    U = (1 / alpha) * (lam1**alpha + lam2**alpha + lam3**alpha - 3)
    pressure = sp.Rational(1, 2) * p * (r * lam2) ** 2 * etap
    elec = -(Phi / (2 * alpha * lam3**2)) * r  # VC electrostatic term (1/(2*alpha) factor).

    # Total potential energy integrand (paper Eq. 9). No leading 2*pi factor.
