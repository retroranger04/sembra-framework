"""Failure-mode helper functions for the voltage-controlled scenario.
...
figures. The scenario (voltage-controlled) is fixed at the package level, so the
electrostatic terms below are the VC forms.
"""
...
from sembra_vc.calibrations import PHI_MAX, PHI_MIN, STRESS_DISPLAY_GRID_SIZE
from sembra_vc.envelope import load_envelopes, predict_p_crit, predict_p_wrinkle
...
SCENARIO_TITLE = "SEMBRA — Voltage-Controlled"
...
def compute_stress(alpha, Phi, p, state, r_grid=None) -> dict:
    ...
    Voltage-controlled form (paper Eq. 17), nondimensionalized by the shear
    ...
    maxwell = Phi / lam3**2  # VC electrostatic contribution
    lam3_a = lam3**alpha
...
def compute_max_electric_field(state) -> float:
    ...
    Voltage-controlled form: ``E_max = lambda_0**2``. ``state`` is
    ``(lambda_0, a, eta_0, b)``.
    """
    lambda_0, _a, _eta_0, _b = state
